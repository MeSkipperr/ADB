...
const { saveTableToNotepad } = require("./notepadManager");
const sendLogClearYoutubeData = require("../email/sendEmail");
...
const processDevices = async () => {
  const statusError = [
    ...
    "Failed to clear Youtube data application",
    "Success",
  ];
  ...
  for (const device of devices) {
    ...
    try {
      ...
      console.log(
        `Trying connect to : ${device.name} | ${device.ipAddress} ...`
      );
      ...
      if (connectOutput.toLowerCase().includes("failed")) {
        ...
        console.error(
          `Cannot Connect to device  ${device.name}: ${connectOutput}`
        );
        continue;
      }

      console.log(
        `Trying to clear Youtube data application : ${device.name} | ${device.ipAddress} ...`
      );
      ...
      if (clearOutput.toLowerCase().includes("failed")) {
        ...
        console.error(
          `Cannot clear youtube data application ${device.name}: ${clearOutput}`
        );
        continue;
      }
  ...
  await saveTableToNotepad(clearDevices, "../email/logClearYoutube.txt");
  await sendLogClearYoutubeData();
};
